# Ralph -- Agentic Coding System for Gemini CLI

Ralph is an autonomous agentic coding system that runs **Gemini CLI in a loop** with **clean context every iteration**. Each iteration is a fresh `gemini` process that picks one user story from `prd.json`, implements it, gets reviewed by a QA Auditor, and commits.
...
```bash
sudo ln -s /path/to/ralph-gemini/gemini-extension/ralph.sh /usr/local/bin/ralph-gemini
```
