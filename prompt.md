...
Before doing anything else, ensure the Python environment is ready:

1. If `pyproject.toml` exists and no `.venv` directory is present, run `uv venv` to create a virtual environment.
2. Run `uv sync` to install all dependencies.
3. Always use `uv run` to execute Python commands (e.g., `uv run python main.py`, `uv run mypy .`). This ensures the correct virtual environment is used without needing to activate it manually.

**Always use `uv` as the package manager.** If you need to add a dependency, use `uv add <package>`. Never use `pip install` directly.
...
If ALL stories are complete and passing, perform the **Final Summary & Documentation** step below before signaling completion.

If there are still stories with `passes: false`, end your response normally (another iteration will pick up the next story).

## Final Summary & Documentation

When all user stories have `passes: true`, you MUST complete this step before finishing:
...
2. **Write a summary section at the end of `progress.txt`** with:
   ```
   ## Final Summary
   - **Total stories completed:** X
   - **Architecture:** Brief description of the system design
   - **Key files:**
     - `file.py` — what it does
     - `file2.py` — what it does
   - **How to run:** The main commands to use this project
   - **Notable decisions:** Any interesting technical choices made during implementation
   ---
   ```

3. **Commit** the documentation changes with: `docs: add final summary and documentation`

4. Then reply with exactly:
